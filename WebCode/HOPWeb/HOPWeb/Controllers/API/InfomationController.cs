...
namespace WQTWeb.Controllers.API
{
    ...
    {
        private IInfomationBll m_InfomationBll;
        public InfomationController()
        {
            m_InfomationBll = DPResolver.Resolver<IInfomationBll>();
        }
        ...
        {
            var pageResult = m_InfomationBll.GetInfomationPager(page, rows, string.IsNullOrEmpty(beginDate) ? null : (DateTime?)DateTime.Parse(beginDate),
